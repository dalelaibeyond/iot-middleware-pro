    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-slate-300 uppercase tracking-tight">Environmental</h3>
        </div>
          ...
          {tempHum.map((data, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-bold uppercase text-slate-500">Zone #{data.sensorIndex}</span>
                {data.temp > 35 && (
                  <span className="text-[10px] font-bold uppercase text-amber-400 animate-pulse">Warning: High</span>
                )}
              </div>
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <div className="text-2xl font-bold tracking-tighter">
                    {data.temp.toFixed(1)}<span className="text-sm font-normal text-slate-500 ml-0.5">°C</span>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                  <div className="flex items-center gap-1.5 text-slate-400">
                    <Wind className="w-3.5 h-3.5" />
                    <span className="text-xl font-bold tracking-tighter">{Math.round(data.hum)}%</span>
                  </div>
                </div>
              </div>
        ...
        {noise.map((n, i) => (
          <div key={i} className="p-4 rounded-xl hardware-card">
             ...
             <div className="flex items-end gap-2">
                <span className="text-3xl font-bold tracking-tighter text-sky-100">{Math.round(n.noise)}</span>
